...
namespace Plugins.WhatAMesh.Scripts.MeshData
{
    /// <summary>
    ...
    public class MeshData
    {
        ...
        public List<Vertex> VerticesInRadius2D(float radius, Vector3 middle)
        {
            List<Vertex> list = new List<Vertex>();

            foreach (Vertex vertex in vertices)
            {
                if (Vector2.Distance(middle, vertex.Position) <= radius)
                {
                    list.Add(vertex);
                }
            }
